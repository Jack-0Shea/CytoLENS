...
def save_qc_image(fname, dapi, fitc, nucleus_labels, cell_territories, cell_results, threshold, output_dir):
    ...
    axes[0].set_title(f"DAPI — {n_nuclei} nuclei (nuclear mask only)", fontsize=9)
    axes[0].axis("off")
    ...
    axes[2].set_title("Overlay — nuclear mask only", fontsize=9)
    axes[2].axis("off")
...
def save_summary_plots(condition_df, output_dir):
    ...
    for ax, metric, ylabel, title in [
        (axes[0], "mean_fitc_255", "Mean FITC intensity (0-255)", "Mean FITC Intensity — Nuclear Mask"),
        (axes[1], "mean_pct_pos",  "FITC-positive cells (%)",     "% FITC-Positive Cells — Nuclear Mask"),
    ]:
        means, sds, xs = [], [], []
